...
{
	// ColdFire
	map(0x00000000, 0x0007ffff).rom();   	// Boot ROM
	map(0x10000000, 0x1003ffff).ram().share("nvram");		// Battery Backed SRAM
	map(0x12000000, 0x1200000f).rw(m_ata, FUNC(ata_interface_device::cs0_swap_r), FUNC(ata_interface_device::cs0_swap_w));
	map(0x12000010, 0x1200001f).rw(m_ata, FUNC(ata_interface_device::cs1_swap_r), FUNC(ata_interface_device::cs1_swap_w));
	
	// FPGA
	map(0x20000000, 0x20003fff).rw(m_fpga, FUNC(pl6fpga_device::dev_r), FUNC(pl6fpga_device::dev_w)); 	// Entire IO system

	// GPU/EXP0/EXP1 Stub - Most bootloaders check this region to see if GPU is installed.
