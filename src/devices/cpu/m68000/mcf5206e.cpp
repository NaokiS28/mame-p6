...
{
	LOGMASKED(LOG_DEBUG, "%s: (Interrupt Pending Register) IMR_r\n", this->machine().describe_context());
	return m_IPR;
}
...
void mcf5206e_device::set_interrupt(IntSrc interrupt){
	...
	if(BIT(m_IMR, interrupt)){
		...
		switch (interrupt){
			...
			case UINT1: 
				ICR = m_ICR[ICR_UART1];
				if (USE_AUTOVEC(ICR)) vector = m_uart[0]->get_irq_vector();
				break;
			case UINT2: 
				ICR = m_ICR[ICR_UART2];
				if (USE_AUTOVEC(ICR)) vector = m_uart[1]->get_irq_vector();
				break;
		...
		if (USE_AUTOVEC(ICR)){
			LOG("%s: Interrupt from %d: ICR: %02x ", this->machine().describe_context(), interrupt, ICR);
			set_input_line_and_vector(INT_LEVEL(ICR), HOLD_LINE, INT_LEVEL(ICR) + 24);
		} else {
			LOG("%s: Interrupt & vector from %d: ICR: %02x, Vector: %02x ", this->machine().describe_context(), interrupt, ICR, vector);
