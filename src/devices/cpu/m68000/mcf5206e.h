...
public:
	...
	// Chipselect
	template<unsigned N> auto cs_cb() { return m_cs_cb[N].bind(); }
...
protected:
	...
	void ICR9_w(uint8_t data) { ICRx_w(9, (data & 0x1F)); }				// Timer 1 *must* use autovector
	void ICR10_w(uint8_t data) { ICRx_w(10, (data & 0x1F)); }			// Timer 2 *must* use autovector
	void ICR11_w(uint8_t data) { ICRx_w(11, (data & 0x1F)); }			// MBUS *must* use autovector
	...
	devcb_write_line::array<8> m_cs_cb;
